...
def update_source(
    ...
    db_source = db.query(models.Source).filter(models.Source.id == source_id, models.Source.owner_id == current_user.id).first()
    if db_source is None:
        raise HTTPException(status_code=404, detail="Source not found")

    update_data = source_update.dict(exclude_unset=True)
    if "url" in update_data and not (update_data["url"].startswith("http://") or update_data["url"].startswith("https://")):
         raise HTTPException(status_code=400, detail="URL must start with http:// or https://")
